...
daily = {
    'description': 'Gain points every 24 hours.',
    'syntax': '`$daily`',
    'example': '`$daily`'
}
register = {
...
def get_command_help(command: str) -> dict | None:
    """
    Returns information for $help <command>.
    """
    if command in command_help_dicts.keys():
        return command_help_dicts[command]
    return None